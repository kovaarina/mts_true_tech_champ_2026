...
> **Санкция.** Попытка решения читать `config/` или `worlds/` (файлы уровня) на зачётном прогоне
> ведёт к **обнулению результата** этого уровня. Файлы закрыты правами; сам факт обращения к ним
> фиксируется судейской обвязкой.
